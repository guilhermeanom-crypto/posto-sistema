...
/** Hash estável do que determina o resultado (perfil + versões). Detecta "precisa recalcular". */
function hashSnapshot(input: DiagnoseInput, rulesVersion: string): string {
  const canonical = JSON.stringify({ perfil: input.perfil, engine: ENGINE_VERSION, rules: rulesVersion })
...
export async function buildDiagnoseInput(
  ...
  const input: DiagnoseInput = {
    ...
    evidencias: {}, // status de conformidade por obrigação → Passo 7
    dataRef,
  }
